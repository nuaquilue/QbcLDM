...
landscape.dyn <- function(scn.name){
  ...
  source("mdl/disturbance.fire2.r")
  ...
  if(is.climate.change){
    load(file=paste0("inputlyrs/rdata/temp85_BFEC", name.resol, ".rdata"))  
    load(file=paste0("inputlyrs/rdata/precip85_BFEC", name.resol, ".rdata"))
  }
  
  # hist(cc.temp[,2]-cc.temp[,21])
  ...
  for(irun in 1:nrun){
    ...
    load(file=paste0("inputlyrs/rdata/land", name.resol, ".rdata"))
     land$IQS[]
     land$IQS[is.na(land$IQS)& land$Temp < -1] <- 80
     land$IQS[is.na(land$IQS)& land$Temp > 1]  <- 60
     land$IQS[is.na(land$IQS)] <- 70
    ...
    for(t in time.seq){
      ...
      if(disturb[1] & t %in% aux.fire.schedule){
        burnt.cells <- disturbance.fire(SPECIES, subset(land, select=c(cell.indx, FRZone)), 
      ...
      ## 2. SBW - spruce budworm outbreak not included in this version
      ...
      if(disturb[4] & t %in% aux.cc.schedule){
        cc.cells <- disturbance.cc(subset(land, select=c(cell.indx, MgmtUnit, SppGrp, Exclus, TSD, TSE, TSF,age.mat,TSPC), (SppGrp!="NonFor" & !is.na(MgmtUnit))), 
                                   cc.step, period.crisis, target.old.pct, diff.prematurite, hor.plan,a.priori, replanif, salvage.rate.event,
                                    salvage.rate.FMU, ref.harv.level, write.tbl.outputs, km2.pixel, irun, t, out.path, 
      ...
      ## 1. FIRE
      if(disturb[1] & sum(burnt.cells)>0){
        ...
        land$SppGrp[land$cell.indx %in% burnt.cells]  <-  forest.trans(subset(land, select=c(cell.indx, SppGrp), cell.indx %in% burnt.cells), 
                       subset(post.fire.reg, select=-age.class, age.class=="adult"), buffer, suitab, dtype="B",persist, p.failure, age.seed, Subopt)
      }
      
      ## 2. SBW
      
      if(disturb[2] & (length(kill.cells)!=0)){
        buffer <- buffer.mig(land[, c("cell.indx", "SppGrp", "CoordX", "CoordY","TSD", "Tcomp")],
        ...
        land$SppGrp[land$cell.indx %in% kill.cells] <- forest.trans(subset(land, select=c(cell.indx, SppGrp), land$cell.indx %in% kill.cells), 
                                                                  post.harvest.reg, buffer, suitab, dtype="C",persist, p.failure, age.seed, Subopt)
      }      
      ...
      ## 4. CLEAR CUTTING
      if(disturb[4] & (length(cc.cells)!=0)){
        buffer <- buffer.mig(land[, c("cell.indx", "SppGrp", "CoordX", "CoordY","TSD", "Tcomp")],
      ...
      if(sum(com.chan.cells)>0) {
        ...
        land$SppGrp[com.chan.cells] <- forest.trans(subset(land, select=c(cell.indx, SppGrp),
                              ...
                              persist, p.failure, age.seed, Subopt)
      }
      ...
      ## For each cell that changes composition, reset TSD at 20 years before maturity
      ...
      if(write.tbl.outputs){
        ...
        write.table(data.frame(run=irun, time=t, spp.suit), 
                    ...
                    append=!(irun==1 & t==5), quote=FALSE, sep="\t", row.names=FALSE, col.names=(irun==1 & t==5))  
        write.table(age.by.spp(land, km2.pixel, min.time.step, irun, t),
                    file = paste0(out.path, "/AgeBySpp.txt"),
                    append=!(irun==1 & t==5), quote=FALSE, sep="\t", row.names=FALSE, col.names=(irun==1 & t==5))  
#        write.table(disturb.by.domain(land, km2.pixel, min.time.step, irun, t),
#                    file = paste0(out.path, "/Disturbances.txt"), 
#                    append=!(irun==1 & t==5), quote=FALSE, sep="\t", row.names=FALSE, col.names=(irun==1 & t==5))   
      ...
      ## Spatial outputs
      if(write.sp.outputs &  irun==1  && t==90){ # 
        ...
        #MASK[!is.na(MASK)] <- as.factor(ifelse(land$TSE< 20,"R",(ifelse(land$TSD>80,"V","J"))))
        #image(MASK,col=c("grey","red","dark green"))

...
buffer.mig <- function(land, target.cells, potential.spp){
  ...
  # buffer.spp <- data.frame(cell.id=target.cells, PotSpp=rep(c("OthCB", "OthDB", "OthCT", "OthDT", "NonFor"), each=length(target.cells)),
  #                          PressBuffer=TRUE)
  buffer.spp <- data.frame(cell.id=target.cells, PotSpp=rep(c("OTH", "NonFor"), each=length(target.cells)),
                           PressBuffer=TRUE)

...
read.state.vars <- function(work.path){  
  ...
  forest.data <- read.dbf("inputlyrs/dbf/points_2k2_ll.dbf")
  ...
  first.time <- T
  if(first.time){
    ZONES <- readOGR("C:/Users/boumav/Desktop/memoire_feux/zones_nuria.shp")
    ...
    rm(aux)
    save(forest.data, file=paste0("inputlyrs/dbf/forest.data.rdata"))
  }
  load("inputlyrs/dbf/forest.data.rdata")
  ...
  ## 6.2. Remove "EcoType" and "other", as these are not anymore needed 
  ...
  land$SoilType <- factor(land$SoilType)  # remove the 'urb' level
    ...
    ##                      ERS - Sugar maple
    ##                      NonFor - Non forest
    ##                      OthCB - Other conifer boreal                
    ##                      OthCT - Other conifer temperate
    ##                      OthDB - Other deciduous boreal
    ##                      OthDT - Other deciduous temperate
    ##                      PET - Trembling aspen
    ##                      SAB - Balsam fir

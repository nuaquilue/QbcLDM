...
age.by.spp2 <- function(land, km2.pixel, min.time.step, irun, t){
  ...
  nMgmtUnit <- length(levels(subland$MgmtUnit))
  ...
  tab_so <- table(subland$MgmtUnit,subland$SppGrp)
                            
  # Complete the data frame with the run and the time step, then write it in a text file
  age.spp2 <- data.frame(rep(irun, nrow(tab_so)), rep(t, nrow(tab_so)), tab_so)
  names(age.spp2) <- c("run","time","UA", "spp", "Area")
  return(age.spp2)

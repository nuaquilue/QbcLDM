...
landscape.dyn <- function(scn.name){
  ...
  for(irun in 1:nrun){
    ...
    load(file=paste0("inputlyrs/rdata/land", name.resol, ".rdata"))
     land$MATU[is.na(land$MATU)& land$SppGr %in% c("ERS","BOJ")] <- 105
     land$MATU[is.na(land$MATU)& land$SppGr %in% c("PET")] <- 65
     #land$MATU[is.na(land$MATU)& land$SppGr %in% c("EPN")] <- 80
     land$MATU[is.na(land$MATU)& land$Temp < -1] <- 80
     land$MATU[is.na(land$MATU)& land$Temp > 1]  <- 65
    ...
    for(t in time.seq){
      ...
      land$TSPC[land$TSD < (land$age.mat/2)] <- 5
      ...
      volume(subset(land, select=c(cell.indx, MgmtUnit, SppGrp, Exclus, TSD,TSC,TSPC)), km2.pixel, min.time.step, irun, t)
